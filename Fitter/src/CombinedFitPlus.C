...
int main(int argc, char *argv[])
{
   ...
   ****************************************************************************************/
   if ( node_name == "Config" ) {
   	...
   	// list of shapes to include
   	if(attr_map.count("includeShapes")>0){
   		stringstream ss(attr_map["includeShapes"]);
   		while(ss.good()){
   			string substr;
   			getline(ss, substr, ',');
   			if(substr!="")
   				myFitMaster->AddShape2Fit(substr);
   		}
   	}
	...
	while (node_child) {
		if ( node_child->GetNodeType() == TXMLNode::kXMLElementNode ) {
			...
			if ( node_child_name == "Sample" ) {
				...
				if( myFitMaster->IsFitShapes()) s->IncludeShapes(myFitMaster->GetShapes2Fit());
				s->FillVector();
   ...
   ****************************************************************************************/
	...
	while (node_child) {
		if ( node_child->GetNodeType() == TXMLNode::kXMLElementNode ) {
			...
			else if (node_child_name == "Argument"){
				map<string, string> attr_map = Util::GetAttributeMap(node_child);
				algo->AddArgument(attr_map["name"], atof(attr_map["arg"].c_str()));
			}
			else {
				MsgWarning(MsgLog::Form("Sample node child %s not recognized! Skipping... " , node_child_name.Data()));
			}
...
MsgInfo( ("\n\n\n\n"+string(100,'=')).c_str());		//output banner
MsgInfo(MsgLog::Form("%56s", "\nInput Info\n"));	//output: Input Info
MsgInfo(string(100,'=').c_str());
myFitMaster->Dump();
MsgLog::Flush();
MsgLog::SetPrintRepetitions(false); // TEMP
myFitMaster->Run();
