...
FitMaster::FitMaster()
{
  ...
  //std::shared_ptr<WeightCalc> dominikwc=std::make_shared<DominikWeightCalc>();
  //Register(dominikwc);
...
void FitMaster::BuildCollapsedEventVector()
{
  ...
  */
  ...
  for ( auto const& it_MCSample : fMCSampleList ) {
    string MCSampleName = it_MCSample->GetName();
    if ( MCSampleName.find("Signal") == std::string::npos ) { // deal with signal later
      // if there is corresponding signal Sample, sum them together
      if ( fBackgroundSignal.find(MCSampleName) != fBackgroundSignal.end() ) {
        for(auto const& it_Signal : fMCSampleList){
          if(it_Signal->GetName() == fBackgroundSignal[MCSampleName]){
            for (Int_t i=0; i<it_MCSample->GetNbins();i++ ) {
      ...
      else { // no corresponding signal Sample, add itself
        for (Int_t i=0; i<it_MCSample->GetNbins();i++ ) {
...
void FitMaster::RecalculateTotalFracErrorMatrix() 
{
  ...
  fTotalFracErrorMatrix->MakePosDef(true); // TEMP!!// dryrun. Only checking
  isTotalRecent = true;
}
...
//fTotalFullErrorMatrix->MakePosDef(true); // dryrun. Only checking
...
} // FitMaster::BuildCollapsedErrorMatrix()
...
{  
  ...
  */
  ...
  for(auto const& rebinInfo : rebin_map){
    bool active = false;
    for(unsigned int i =0 ; i<rebinInfo.second.size();i++){
      if(rebinInfo.second.at(i)!=0) active = true;
    }
    fSampleActive[rebinInfo.first] = active;
  }
  ...
  Int_t n_bins_new = 0;
  for ( auto const& MCSample : fMCSampleList ) {
    map<string, vector<Int_t> >::const_iterator rebinmap_it  = rebin_map.find(MCSample->GetName());
    if ( rebinmap_it == rebin_map.end() ) {
      MsgError("FitMaster::Fatal Error!");
      MsgFatal("FitMaster::Rebin map does not have new bins for "+MCSample->GetName() +" sample");
      exit (0);
    }
...
Double_t FitMaster::GetChi2()
{
  ...
  if(fFitShape) chi2 += GetShapeChi2();
  return chi2;
}
...
Double_t FitMaster::GetLSNDChi2()
{
  ...
  double chi2 = 0.;
  double prob = 0.;
  string fakeLSND = MsgLog::Form("LSND Fake %05d: ", fCurrentFakeDataFitN);
...
Double_t FitMaster::GetShapeChi2(){
  // calculate the shape chi2
  // assumption: 
  // 1. calculate the minimum shape chi2 for each data set
  // 2. calculate only nue(bar) ones
  // 3. nue/nuebar are scaled separately
  // 4. each shape is calculated differently then sum up
  // Method:
  // 1. Loop through shape
  // 2. Loop through data (Nue Data) ->ignore numu ones
  // 3. find the corresponding MC (Nue MC)
  // 4. find the corresponding Signal (Nue Signal)
  // 5. sum up the MC + Signal
  // 6. Add to the chiSq
  // Furtheremore, to minimize chiSq = 2 * sum(aPi - di + di*log(di/aPi))
  // a is equal to sum(di)/sum(Pi)
  Double_t chi2=0.;
  // 1. loop through shape first
  for(string shapeName : fShapes2Fit){ 
    vector<Double_t> vdata; // sum of data
    vector<Double_t> vpred; // sum of MC
    vdata.clear();
    vpred.clear();
    // 2. Loop through data (Nue Data)
    for(auto const& DataSample : fDataSampleList){
      string dataName = DataSample->GetName();
      if(dataName.find("Numu")!=string::npos) continue; //ignore numu
      if(fSampleActive[dataName]==false) continue;      // ignore non-active ones(rebins are all 0s)

      string MCName = dataName;
      MCName.replace(MCName.find("Data"),4,"MC"); // e.g. Nue MC
      if(fBackgroundSignal.find(MCName)==fBackgroundSignal.end()){
        MsgError( ("Cannot find "+MCName+" in fBackgroundSignal").c_str());
        continue;
      }
      string SignalName = fBackgroundSignal[MCName];
      // obtain MCSample and SignalSample
      auto const& MCSample     = *(find_if(fMCSampleList.begin(), fMCSampleList.end(), 
                                         [&](shared_ptr<Sample> sample){return sample->GetName()==MCName;}));
      auto const& SignalSample = *(find_if(fMCSampleList.begin(), fMCSampleList.end(), 
                                         [&](shared_ptr<Sample> sample){return sample->GetName()==SignalName;}));

      double sumData=0, sumPred=0;
      int nbins = DataSample->GetNbins(shapeName);
      for(int i =0; i<nbins;i++){
        vdata.push_back(DataSample->GetBinContent(i, shapeName));
        vpred.push_back(MCSample->GetBinContent(i, shapeName)+SignalSample->GetBinContent(i, shapeName));
        sumData += vdata.at(i);
        sumPred += vpred.at(i);
      }
      double scale = sumData / sumPred;
      for(int i=0;i<nbins;i++){
        if(vdata.at(i)>0) // use poisson equation
          chi2 += 2. * (scale*vpred.at(i) - vdata.at(i) + vdata.at(i)*log(vdata.at(i)/(scale*vpred.at(i))));
        else // simply the vpred
          chi2 += 2 * scale * vpred.at(i);
      } 
...
void FitMaster::Run()
{
  ...
  if (fDoFakeDataFits && fNfakeDataFits>0) {
    ...
    // ---------------------------
      ...
      //fTotalFullErrorMatrix->MakePosDef(true); // true for dry run, not making pos deinite
      ...
      fCollapsedErrorMatrix->MakePosDef(true); // not dry run here! Needs to be real!
    ...
    for (fCurrentFakeDataFitN=0; fCurrentFakeDataFitN<fNfakeDataFits; fCurrentFakeDataFitN++) {
      ...
      Int_t firstbin=0;
      for(auto & data_sample : fDataSampleList){
        int nbins = data_sample->GetNbins();
...
data_sample->Dump();
      ...
      for ( auto const& it : fMCSampleList ) {
        ...
        if (name.find("Signal") != std::string::npos){  // e.g. Nue Signal -> Nue Data
          ...
          continue; // using fCollapsedErrorMatrix
        }
        ...
        // fDataSampleList is empty, therefore, we have to add sample into it
        // it should not "already_have" a sample here!!!
        ...
        if ( already_have ) {
          MsgError("Should not already_have"+name); // using fCollapsedErrorMatrix
          std::vector<Double_t> data;
          data.resize(nbins);	
      ...
      /* if generating fake data with fTotalFullErrorMatrix 
      fDataSampleList.erase(fDataSampleList.begin(),fDataSampleList.end());
      Int_t firstbin=0;
      for ( auto const& it : fMCSampleList ) {
        Int_t nbins      =it->GetNbins();
      ...
      // prepare random for LSND
      rnd3=new TRandom3();
      rnd3->SetSeed(fFakeDataRandomSeed);
