...
void AlgoFit::Run()
{
  ...
  fHchi2Det    = Get2dHist("chi2Det"   ,fParName[1],fParName[0],fLogScale[1],fLogScale[0]);
  fHchi2raster = Get2dHist("chi2raster",fParName[1],fParName[0],fLogScale[1],fLogScale[0]);
  fHchi2Shape  = Get2dHist("chi2Shape" ,fParName[1],fParName[0],fLogScale[1],fLogScale[0]); // note fHchi2 contains shape already

  // for resonance model now: 3 parameters
  if(fPar.size()==3) {
    fIs3D = true;
    ...
    fHchi23DDet= Get3dHist("chi23DDet",fParName[1],fParName[0], fParName[2], fLogScale[1],fLogScale[0], fLogScale[2]);
    for(int binX=1; binX<= fHchi2->GetNbinsX(); binX++){
      for(int binY=1; binY<= fHchi2->GetNbinsY(); binY++){
        fHchi2->SetBinContent(binX, binY, 99999999);      // 2D version is 3D version collapsed on third
        fHchi2Shape->SetBinContent(binX, binY, 99999999); // get smallest
      }
    }
  }
  ...
  // argument(no scan) is treated as parameter for FitMaster
  for(unsigned int iArg=0; iArg<fArgName.size(); iArg++){
    fFitMaster->SetParameter(fArgName.at(iArg), fArg.at(iArg));
  }
  ...
  while (!converged) {
    ...
    oss << endl;
    oss << "Min chi2               = "<<setw(10)<<fMinChi2<<endl;
  ...
  if (fFitMaster->IsFakeDataFit()) {
    ...
    if (fDumpHistograms && fHchi2->GetXaxis()->GetNbins()>1) {
      ...
      fHchi2Det->SetName(Form("%s_%i", fHchi2Det->GetName(),fFitMaster->GetCurrentFakeDataFitN()));
      fHchi2->Write();
      fHchi2Det->Write();
      if(fFitMaster->IsFitShapes()){
        fHchi2Shape->SetName(Form("%s_%i",fHchi2Shape->GetName(),fFitMaster->GetCurrentFakeDataFitN()));
        fHchi2Shape->Write();
  ...
  // -------------------
    ...
    for ( auto const& data_it : data_list ) {
      // skip sample that wasn't used
      if(fFitMaster->SampleActive(data_it->GetName())==false) 
        continue;
      TH1D* h=data_it->GetHistogram();          // save EnuQE histogram
      if (fDumpHistograms && h!=0) h->Write();
      delete h;
      for(string shapeName : data_it->GetShapes2Fit()){
        h = data_it->GetHistogram(shapeName);   // save shape histograms
        if (fDumpHistograms && h!=0) h->Write();
        delete h;
      }
    }
    ...
    for ( auto const& signal_it : signal_list ) {
      if(fFitMaster->SampleActive(signal_it->GetName())==true)
  ...
  if (fFitMaster->GetCurrentFakeDataFitN()==0) {
    ...
    for ( auto const& mc_it : mc_list ) {
      if (fDumpHistograms && fFitMaster->SampleActive(mc_it->GetName())==true) 
        mc_it->WriteHistogramsToFile(f);
    }
...
void AlgoFit::Loop(Int_t n)
{
  ...
  for (fBin[n] = 0; fBin[n] < fNbins[n]; fBin[n]++) {
    if ( fLogScale[n] ) 
      ...
      fPar[n] = fParMin[n]+dx*fBin[n];
    fFitMaster->SetParameter(fParName[n], fPar[n]);
    //if(n==0) {
    if(fabs(fPar[n])>0.001){
      MsgDebug(1,MsgLog::Form("\nSetting %13s to bin %3d/%3d at %10.5f", fParName[n].c_str(), fBin[n], fNbins[n], fPar[n]));
    }
    else{
      MsgDebug(1,MsgLog::Form("\nSetting %13s to bin %3d/%3d at %10.2e", fParName[n].c_str(), fBin[n], fNbins[n], fPar[n]));
    }
    MsgLog::Flush();
    //}
    ...
    else 
    {
      ...
      if ( !fDoingRaster) { // fDoingRaster == False
        ...
        Double_t chi2Shape = fFitMaster->GetShapeChi2();
        Double_t chi2Det   = fFitMaster->GetLogDeterminant();
        if(fIs3D){
          fHchi23D->SetBinContent(fBin.at(1)+1, fBin.at(0)+1, fBin.at(2)+1, chi2);   // the order is to follow the dm2, s2t, Eres order
          fHchi23DDet->SetBinContent(fBin.at(1)+1, fBin.at(0)+1, fBin.at(2)+1, chi2Det);   // the order is to follow the dm2, s2t, Eres order
          ...
          if(tempChi2>chi2){
            ...
            fHchi2Det->SetBinContent(fBin.at(1)+1, fBin.at(0)+1, chi2Det);        // all different Eres'
          }                                                                       
        }
        else{ // 2D
          fHchi2->SetBinContent(fBin.at(1)+1,fBin.at(0)+1, chi2);
          fHchi2Det->SetBinContent(fBin.at(1)+1, fBin.at(0)+1, chi2Det);
          fHchi2Shape->SetBinContent(fBin.at(1)+1,fBin.at(0)+1, chi2Shape);
        }
