...
public :
  ...
  Double_t GetShapeChi2(); // get the chiSq from shape(s)
  ...
  // shape
  void AddShape2Fit(std::string shape){ fFitShape=true; fShapes2Fit.push_back(shape);};
  std::vector<std::string> GetShapes2Fit(){return fShapes2Fit;};
  bool IsFitShapes() {return fFitShape;}
  bool SampleActive(std::string sampleName){return fSampleActive[sampleName];};
...
private:
  ...
  // include shape fittings
  bool                         fFitShape;
  std::vector<std::string>     fShapes2Fit;
  std::map<std::string, bool>  fSampleActive; // check whether sample is active
