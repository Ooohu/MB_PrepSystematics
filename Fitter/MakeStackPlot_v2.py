...
class StackPlots(object):
   ...
   def Plot(self,out_stem, fileType="png", errorType = "stat"):
      ...
      self.data.GetYaxis().SetTitleOffset(1.1)
      ...
      self.data.GetYaxis().SetTitleOffset(1.3)
      self.data.DrawCopy("PE1")
      self.signal.DrawCopy("samehist")
...
if __name__ == "__main__":
   ...
   #numode histogram
#   nue = StackPlots("./output/fit_oct20_histNunubar_nueUpdated.root", printFix= True, fix_last_bin=True,bar = "")
   nue = StackPlots(sys.argv[1],printFix= True, fix_last_bin=True,bar = "") #Keng v2 update: read file from argument;
   nue.printBinContent()
   #nue.printCombinedExcess()   
   nue.Plot("figures/oct20/histNunubar/histNunubar", "pdf")
