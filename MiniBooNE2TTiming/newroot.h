...
//   solution for single root file;
	global_index = 1;
	file_order = {"_","_1","_2","_3","_4","_5","_6","_7","_8","_9","_10"};
...
//   }
	...
	if (tree == 0) {
		...
		while ( (Onefile = (TSystemFile*)next())){//go through file names under Onefile iterator
			...
			if(!current_file_name.Contains(filelist[index]+file_order[global_index])) continue;//pick file with prefix
			if(current_file_name.Length() != 6 + (filelist[index]+file_order[global_index]).Length()) continue;//not files with same prefix
